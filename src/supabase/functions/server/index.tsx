...
async function cleanupDatabase() {
  ...
  const results = { removed: [] as string[], kept: [] as string[], errors: [] as string[] };
  try {
    const allKeys = await kv.getByPrefix("");
    console.log(`📊 Found ${allKeys.length} total keys in database`);
    const keepPatterns = ["^website_content$", "^pricing_config$", "^db_initialized$", "^booking_current_prefix$", "^booking_used_prefixes$", "^HOP-[A-Z]{2}\\d{4}$", "^availability_\\d{4}-\\d{2}-\\d{2}$", "^stripe_", "^pwa_icons_deployed$", "^pwa_icon_"];
    const removePatterns = ["^comprehensive_content$", "^checkin_", "^checkins_", "^destination_", "^destination_log_", "^analytics_"];
    for (const key of allKeys) {
      const shouldKeep = keepPatterns.some(pattern => new RegExp(pattern).test(key));
      const shouldRemove = removePatterns.some(pattern => new RegExp(pattern).test(key));
    ...
    console.log(`\n📊 Cleanup Summary: ✅ Kept: ${results.kept.length} | 🗑️  Removed: ${results.removed.length} | ❌ Errors: ${results.errors.length}`);
    return results;
  } catch (error) {
...
async function removeLegacyBranding() {
  ...
  try {
    ...
    if (content && (content.company?.email?.includes("gosintra.com") || content.company?.name?.includes("Go Sintra") || content.seo?.home?.title?.includes("Go Sintra"))) {
...
app.get("/make-server-3bd0ade8/pricing", async (c) => {
  try {
    ...
    if (pricing) {
      ...
      console.log(
        "ℹ️ No pricing found in database, returning defaults",
      );
      ...
      return c.json({ success: true, pricing: defaultPricing });
    }
  } catch (error) {
...
// Verify Booking with Last Name for User Login (Temporary Profile System)
app.post("/make-server-3bd0ade8/verify-booking-login", async (c) => {
  try {
    const { bookingId, lastName } = await c.req.json();
    
    console.log('🔐 User login attempt:', { bookingId });
    
    if (!bookingId || !lastName) {
      return c.json({ success: false, error: "Booking ID and last name are required" }, 400);
    }

    // Try to get booking directly (handles all formats: HOS-####, AA-####, etc.)
    let booking = await kv.get(bookingId.trim().toUpperCase());
    
    // Backwards compatibility: try old formats
    if (!booking) {
      const cleanId = bookingId.replace(/^(HOS-|booking_)/i, '');
      booking = await kv.get(`booking_${cleanId}`);
    }
    ...
    return c.json({
      success: true,
      booking: {
        bookingId: booking.id || bookingId.trim().toUpperCase(),
        customerName: bookingName,
        customerEmail: booking.contactInfo?.email || "",
        customerPhone: booking.contactInfo?.phone || "",
        passes: booking.passengers?.length || 1,
        visitDate: booking.selectedDate || booking.visitDate || new Date().toISOString(),
      },
    });
  } catch (error) {
