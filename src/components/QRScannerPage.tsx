...
export function QRScannerPage({
  ...
  const startScanning = async () => {
    try {
      ...
      if (videoRef.current) {
        ...
        const hasSeenCameraTip = localStorage.getItem(
          "camera-permission-tip-seen",
        );
        if (!hasSeenCameraTip) {
          toast.success(
            "Camera activated! Permission saved for future use.",
            { duration: 3000 },
          );
      ...
      toast.error(
        "Unable to access camera. Please grant camera permissions and ensure HTTPS is enabled.",
      );
    }
  };
      ...
      if (result.success) {
        ...
        try {
          ...
          if (driverSession) {
            ...
            const [bookingId, passengerIndex] =
              qrData.split("|");
  ...
  const checkInPassenger = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-3bd0ade8/checkin`,
        {
          ...
          body: JSON.stringify({
            ...
            passengerIndex:
              scanResult.booking.passengerIndex || 0,
      ...
      if (result.success) {
        toast.success(
          `Passenger checked in to ${selectedDestination}!`,
        );
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>
              Scan passenger QR codes to verify passes and check
              in
            </CardDescription>
          </CardHeader>
            ...
            <div className="relative overflow-hidden rounded-lg bg-black">
              ...
              {!scanning && !scanResult && (
                <div className="flex min-h-[300px] flex-col items-center justify-center gap-4 p-8 text-center">
                  ...
                  <p className="text-muted-foreground">
                    Camera is off. Click the button below to
                    start scanning.
                  </p>
                </div>
              )}
              ...
              {scanning && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div
                    className="h-48 w-48 rounded-lg border-4 border-accent shadow-lg"
                    style={{
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <p className="font-medium">
                              {scanResult.booking.passType}
                            </p>
                            <p className="text-muted-foreground">
                              {scanResult.booking.numPasses}{" "}
                              {scanResult.booking.numPasses ===
                              1
                                ? "pass"
                                : "passes"}
                              {scanResult.booking.guidedTour &&
                                " • Guided Tour"}
                            </p>
                          </div>
                        </div>
                        ...
                        {scanResult.booking.checkIns &&
                          scanResult.booking.checkIns.length >
                            0 && (
                            <div className="border-t pt-3">
                              ...
                              <div className="space-y-1">
                                {scanResult.booking.checkIns.map(
                                  (checkin, index) => (
                                    <p
                                      key={index}
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                {scanResult?.booking?.customerName} - Where are
                they going?
              </DialogDescription>
            </DialogHeader>
